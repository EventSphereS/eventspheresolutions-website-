...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled
        ? 'bg-white/90 backdrop-blur-md border-b border-gray-100/80 shadow-sm shadow-black/5'
      ...
      <div className="w-full px-4 sm:px-8 lg:px-16 py-1.5 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 lg:gap-7">
          ...
          <a href="https://crm.eventspheresolutions.com/signin" target="_blank" rel="noopener noreferrer" className="text-[#6a256f] border border-[#6a256f] hover:bg-[#6a256f] hover:text-white font-semibold text-sm px-4 py-2 rounded-full transition-colors">Log In</a>
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-gray-100 px-6 py-6 flex flex-col gap-5 shadow-lg">
          ...
          <a href="https://crm.eventspheresolutions.com/signin" target="_blank" rel="noopener noreferrer" className="text-[#6a256f] border border-[#6a256f] font-semibold text-base text-center py-2 rounded-full" onClick={() => setOpen(false)}>Log In</a>
